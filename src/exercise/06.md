...
We've already memoized the `<Grid />` and `<CellImpl />` components, but it's still
...
you'll notice that all the `CellImpl`s are re-rendering but only the one that was
...
Conceptually, each `CellImpl` in our example represents a different component you
...
So what if we make the `CellImpl` actually accept a `cell` prop and then put a
middle-man component in there that does the actual consuming of `useAppState()`.
We could rename the `CellImpl` component to `CellImpl` (Impl is short for
"implementation"). And then the "middle-man" component could be called `CellImpl`
(which is the one that we actually expose for use). That way if there's a state
update, `CellImpl` is re-rendered, and it forwards the `cell` to `CellImpl` which is
...
This whole time we've been pretending that each `CellImpl` actually represented a
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Performance%20%E2%9A%A1&e=06%3A%20Fix%20%22perf%20death%20by%20a%20thousand%20cuts%22&em=director093%40gmail.com).
